...
import com.dmc.eirs.model.CameraInfo
import kotlinx.coroutines.flow.flow


class CameraDataProvider(private val context: Context) {
    fun getCameraInformation() = flow {
        emit(CameraInfo(
                isCameraAvailable = isCameraAvailable,
                isFlashAvailable = isFlashAvailable,
                cameraIds = cameraIds,
                numberOfCameras = getNumberOfCameras(),
                antibandingModes = getAntibandingModes(cameraIds?.first() ?: String()),
                aberrationModes = getAberrationModes(cameraIds?.first() ?: String()),
                autoExposureModes = getAutoExposureModes(cameraIds?.first() ?: String()),
                autoFocusModes = getAutoFocusModes(cameraIds?.first() ?: String()),
                effects = getEffects(cameraIds?.first() ?: String()),
                whiteBalanceModes = getWhiteBalanceModes(cameraIds?.first() ?: String()),
                videoStabilizationModes = getVideoStabilizationModes(cameraIds?.first()
                        ?: String()),
                maximumAutoExposureRegions = getMaximumAutoExposureRegions(cameraIds?.first()
                        ?: String()),
                maximumAutoFocusRegions = getMaximumAutoFocusRegions(cameraIds?.first()
                        ?: String()),
                maximumAutoWhiteBalanceRegions = getMaximumAutoWhiteBalanceRegions(cameraIds?.first()
                        ?: String()),
        )
        )
    }
     ...
     */
    private val cameraIds: Array<String>?
        get() {
     ...
     */
    private fun getAntibandingModes(cameraId: String): IntArray? {
     ...
     */
    private fun getAberrationModes(cameraId: String): IntArray? {
        return getCharacteristics(
                cameraId,
     ...
     */
    private fun getAutoExposureModes(cameraId: String): IntArray? {
     ...
     */
    private fun getAutoFocusModes(cameraId: String): IntArray? {
     ...
     */
    private fun getEffects(cameraId: String): IntArray? {
     ...
     */
    private fun getWhiteBalanceModes(cameraId: String): IntArray? {
     ...
     * ON--------> 2
     */
    private fun getVideoStabilizationModes(cameraId: String): IntArray? {
        return getCharacteristics(
                cameraId,
     ...
     * @return int
     */
    private fun getMaximumAutoFocusRegions(cameraId: String): Int {
     ...
     * @return int
     */
    private fun getMaximumAutoExposureRegions(cameraId: String): Int {
     ...
     * @return int
     */
    private fun getMaximumAutoWhiteBalanceRegions(cameraId: String): Int {
