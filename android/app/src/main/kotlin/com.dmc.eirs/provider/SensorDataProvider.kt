...
import com.dmc.eirs.model.SensorDetails
import com.dmc.eirs.model.SensorInfo
import kotlinx.coroutines.flow.flow


class SensorDataProvider(private val context: Context) {

    fun getSensorInfo() = flow {
        emit(
                SensorInfo(
                        sensorList = sensorList,
                        totalNumberSensor = getTotalNumberOfSensors(),
                        sensorDetails = getSensorDetails()
                )
        )
    }

    private fun getSensorDetails(): ArrayList<SensorDetails> {
        val sensorDetails = arrayListOf<SensorDetails>()
        sensorList.forEach {
            sensorDetails.add(SensorDetails(getSensorName(it), getSensorVendor(it), getSensorVersion(it), getSensorResolution(it), getSensorPower(it), getSensorMaximumRange(it)))
        }
     ...
     */
    private fun getTotalNumberOfSensors(): Int {
     ...
     */
    private fun getSensorVendor(sensor: Sensor): String {
     ...
     */
    private fun getSensorVersion(sensor: Sensor): Int {
     ...
     */
    private fun getSensorResolution(sensor: Sensor): Float {
     ...
     */
    private fun getSensorPower(sensor: Sensor): Float {
        return sensor.power
    }
     ...
     */
    private fun getSensorMaximumRange(sensor: Sensor): Float {
